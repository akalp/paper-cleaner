...
Use local filesystem storage backed by a local SQLite metadata database.
...
A missing preview file returns `404`; previews are regenerated by mutations, not on read.

#### `GET /api/documents/{document_id}/source`

Return the EXIF-normalized source image as a PNG (downscaled to at most 1600px).

#### `GET /api/documents/{document_id}/export/image`

Return the current rendered document as a PNG download.

### Document response scale factors

Document responses include:

- `source_scale`: scale from full-resolution normalized coordinates to served source pixels
- `preview_scale`: scale from crop-space coordinates to served final preview pixels

Both currently equal `min(1, 1600 / width, 1600 / height)`.
...
Each erase path represents a polygon or user-drawn region and carries a stable `id`.
When applied:
...
Erase regions are preserved across perspective and crop changes; they are only replaced by a
new `POST .../erase` call.
